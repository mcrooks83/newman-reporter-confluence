    ...
    confluence.getContentByPageTitle(options.spaceId, pageTitle, (err, res) => {
        ...
        switch (res.size) {
            ...
            case 0:
                ...
                break;
            default:
                throw new Error(`Got too many results form Confluence getContent API: ${JSON.stringify(res, null, 2)}`);
        }
    });
}
...
PostmanConfluenceReporter = function (newman, options, collectionRunOptions) {
    ...
    newman.on('beforeDone', function () {
        const items = {},
            ...
            // added a unique indentifier as to not overrite reports and keep an audit trail of tests
            pageTitle = this.summary.collection.name + ' ' + Date.now();
